...
const BatchActionModal = ({ isOpen, onClose, fetchWarehouse, warehouseId }) => {
  ...
  const removeFromSelection = (id) => {
    ...
    if (selectedBlocks.length === 1) setIsSelectingParent(false);
  };
        ...
        >
          ...
          >
                    ...
                    >
                      {!isSelectingParent && (
                        <button onClick={() => toggleSelection(bloc._id)}>
                          {selectedBlocks.includes(bloc._id) ? (
                        ...
                        <div>
                          <h3 className="font-semibold">
                            ...
                            <span className="text-gray-500 text-sm ml-2">
                              inside
                              {bloc.parent ? " " + bloc.parent.name : " root"}
                            </span>
                          </h3>
            ...
            <div className="w-1/3 p-6 flex flex-col justify-between">
              <h2 className="text-lg font-semibold mb-4">Blocs sélectionnés</h2>
              <ul className="text-gray-600 text-sm space-y-2 overflow-scroll max-h-[60vh]">
                {selectedBlocks.map((id) => {
                  const bloc = results.find((b) => b._id === id);
                  return (
                    <div
                      key={id}
